  ...
  while(true) {
    ...
    */
    ...
    if (gameData->ragData != nullptr) {
      const GameRAGData* ragData = (const GameRAGData*)gameData->ragData;
      int deepAttempts = 0;
      int deepSuccess = 0;
      for(const PerMoveRAGData& moveData : ragData->flagged_positions) {
        if(!moveData.deep_result.status.empty() || moveData.deep_result.visits > 0)
          deepAttempts++;
        if(moveData.deep_result.available)
          deepSuccess++;
      }
      if(logger != NULL) {
        logger->write(Global::strprintf("[DATAGO] Inline deep summary: game=%s flagged=%d deep=%d success=%d",
                                        ragData->game_id.c_str(),
                                        (int)ragData->flagged_positions.size(),
                                        deepAttempts,
                                        deepSuccess));
      }
      ...
      writeCompleteRAGDataJSON(komi, board_size, rules, ragData, gameData);
