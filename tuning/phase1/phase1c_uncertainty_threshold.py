...
class Phase1cTuner:
    ...
    def find_optimal_threshold(self, configs: List[StorageThresholdConfig], 
        ...
        # Step 1: Estimate percentiles
        percentile_map = self.estimate_percentiles_from_database(ground_truth_db_path)
        
        # Step 2: Generate threshold configs
        configs = []
        for percentile, threshold in percentile_map.items():
            config = UncertaintyThresholdConfig(
        ...
        print(f"\nTesting {len(configs)} threshold configurations...")
        ...
        for i, config in enumerate(configs, 1):
            print(f"\n[{i}/{len(configs)}] Testing threshold configuration...")
            ...
            if metrics.win_rate > best_win_rate:
                ...
                print(f"  ✓ New best win rate: {best_win_rate:.3f}")
        
        # Step 4: Save all results
        results_file = self.output_dir / "phase1c_results.json"
        with open(results_file, 'w') as f:
            json.dump(self.results, f, indent=2)
        print(f"\nSaved all results to {results_file}")
        
        # Step 5: Save best config
        best_config_file = self.output_dir / "best_threshold_config.json"
        ...
        self.plot_threshold_comparison(configs, self.results)
        
        # Summary
        print("\n" + "="*80)
        print("PHASE 1C TUNING COMPLETE")
        print("="*80)
        print(f"Best threshold: Top {best_config.threshold_percentile}% (abs={best_config.absolute_threshold:.4f})")
        print(f"Win rate: {best_metrics.win_rate:.3f}")
        print(f"RAG query rate: {best_metrics.rag_query_rate:.3f}")
        print(f"High relevance hits: {best_metrics.high_relevance_hits}")
        print(f"Avg relevance score: {best_metrics.avg_relevance_score:.3f}")
        print("="*80)
